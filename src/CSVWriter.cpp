...
void CSVWriter::writeHeader(std::ostream& out) {
    out << "Flow ID,Src IP,Src Port,Dst IP,Dst Port,Protocol,"
        ...
        << "Fwd Pkt Len Min,Fwd Pkt Len Max,Fwd Pkt Len Mean,Fwd Pkt Len Std,Fwd Pkt Len Var,"
        << "Bwd Pkt Len Min,Bwd Pkt Len Max,Bwd Pkt Len Mean,Bwd Pkt Len Std,Bwd Pkt Len Var,"
        << "Pkt Len Min,Pkt Len Max,Pkt Len Mean,Pkt Len Std,Pkt Len Var,"
        << "Fwd IAT Total,Fwd IAT Min,Fwd IAT Max,Fwd IAT Mean,Fwd IAT Std,Fwd IAT Var,"
        << "Bwd IAT Total,Bwd IAT Min,Bwd IAT Max,Bwd IAT Mean,Bwd IAT Std,Bwd IAT Var,"
        << "Flow IAT Total,Flow IAT Min,Flow IAT Max,Flow IAT Mean,Flow IAT Std,Flow IAT Var,"
...
bool CSVWriter::writeFlowRow(std::ostream& out, const BasicFlow& flow) {
    ...
    // Packet Size Profiles - Fwd (5 features)
    out << (has_fwd_stats ? formatJavaLikeDouble(flow.fwd_pkt_stats.min) : std::string("0")) << ','
        ...
        << (has_fwd_stats ? formatJavaLikeDouble(flow.fwd_pkt_stats.variance()) : std::string("0")) << ',';

    // Packet Size Profiles - Bwd (5 features)
    out << (has_bwd_stats ? formatJavaLikeDouble(flow.bwd_pkt_stats.min) : std::string("0")) << ','
        ...
        << (has_bwd_stats ? formatJavaLikeDouble(flow.bwd_pkt_stats.variance()) : std::string("0")) << ',';

    // Packet Size Profiles - Total (5 features)
    ...
    out << (has_fwd_iat ? formatJavaLikeDouble(flow.forward_iat.sum) : std::string("0")) << ','
        ...
        << (has_fwd_iat ? formatJavaLikeDouble(flow.forward_iat.variance()) : std::string("0")) << ',';

    // IAT Profiles - Bwd (6 features)
    out << (has_bwd_iat ? formatJavaLikeDouble(flow.backward_iat.sum) : std::string("0")) << ','
        ...
        << (has_bwd_iat ? formatJavaLikeDouble(flow.backward_iat.variance()) : std::string("0")) << ',';

    // IAT Profiles - Flow (6 features)
    out << (has_flow_iat ? formatJavaLikeDouble(flow.flow_iat.sum) : std::string("0")) << ','
        ...
        << (has_flow_iat ? formatJavaLikeDouble(flow.flow_iat.variance()) : std::string("0")) << ',';
